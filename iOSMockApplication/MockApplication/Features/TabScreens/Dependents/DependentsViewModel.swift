...
class DependentsViewModel: ObservableObject {
    ...
    /// - Parameters:
    ///   - contact: The contact to get the status color for
    ///   - colorScheme: The current color scheme
    /// - Returns: The status color
    func statusColor(for contact: Contact, colorScheme: ColorScheme) -> Color {
        if contact.manualAlertActive {
            return .red
        } else if contact.isNonResponsive || isCheckInExpired(contact) {
            return colorScheme == .light ? Color(UIColor.systemOrange) : .yellow
        } else {
            return .secondary
        }
    }
    ...
    /// - Parameters:
    ///   - contact: The contact to get the card background for
    ///   - colorScheme: The current color scheme
    /// - Returns: The card background color
    func cardBackground(for contact: Contact, colorScheme: ColorScheme) -> Color {
        if contact.manualAlertActive {
