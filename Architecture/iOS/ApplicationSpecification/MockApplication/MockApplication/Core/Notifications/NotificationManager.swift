...
class NotificationManager {
    ...
    func showSilentLocalNotification(
        ...
        if !isAuthorized {
            ...
            requestAuthorization { granted in
                if granted {
                    self.scheduleSilentNotification(title: title, body: body, type: type, delay: delay, completion: completion)
                } else {
                    print("Notification authorization denied")
                    completion?(NSError(domain: "NotificationManager", code: 1, userInfo: [NSLocalizedDescriptionKey: "Notification authorization denied"]))
                }
            ...
            scheduleSilentNotification(title: title, body: body, type: type, delay: delay, completion: completion)
        }
    }
